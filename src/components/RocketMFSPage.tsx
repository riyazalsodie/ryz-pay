...
const RocketMFSPage: React.FC<RocketMFSPageProps> = ({ onBack, onCancel, onVerify, gateway, transaction }) => {
  const [transactionId, setTransactionId] = useState('')
  const [isSubmitting, setIsSubmitting] = useState(false)
  const { language, setLanguage } = useStore()

  const recipientNumber = gateway?.config?.walletNumber || '017629050134'
