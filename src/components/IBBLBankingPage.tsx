...
const IBBLBankingPage: React.FC<IBBLBankingPageProps> = ({ onBack, onCancel, onSubmit, gateway }) => {
  ...
  const bankAccountNumber = gateway?.config?.accountNumber || '20502130100209900'
  const bankBranch = gateway?.config?.branchName || 'Gulshan Circle-1'
  const bankRouting = gateway?.config?.routingNumber || '125261828'
  const bankSwift = gateway?.config?.swiftCode || 'IBBLBDDH110'
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-sm dark:bg-black/50">
          <div className="bg-white rounded-lg p-8 shadow-2xl dark:bg-[#111] dark:border dark:border-white/10">
            <div className="flex flex-col items-center">
              <div className="w-16 h-16 border-4 border-[#388E3C] border-t-transparent rounded-full animate-spin"></div>
              <p className="mt-4 text-slate-600 font-bangla dark:text-gray-300">আপলোড করা হচ্ছে...</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="up-container max-w-md overflow-hidden mx-auto p-8 sm:relative sm:bg-white sm:rounded-lg sm:shadow-lg sm:shadow-[#0057d0]/10 sm:min-w-[650px] sm:flex sm:flex-wrap dark:bg-black dark:border dark:border-white/10 dark:shadow-none">
        ...
        <div className="w-full">
          <div className="flex flex-col sm:flex-row flex-wrap sm:mt-5 sm:justify-between sm:items-center">
            ...
            <div className="bg-white shadow shadow-[#0057d0]/5 rounded-lg px-5 py-3 sm:h-[85px] flex items-center sm:w-[70%] sm:shadow-none sm:ring-1 sm:ring-[#0057d0]/10 dark:bg-black dark:border dark:border-white/10 dark:shadow-none dark:ring-0">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[#94a9c7] text-sm font-bangla dark:text-gray-400">ইনভয়েস আইডিঃ</span>
                <p className="text-[#6D7F9A] text-sm select-all dark:text-gray-400">{invoiceId}</p>
              </div>
            </div>
          ...
          <form onSubmit={handleSubmit} className="mt-3">
            <div className="bg-[#388E3C] p-5 mt-3 rounded-lg overflow-auto">
              <div className="text-center">
                <h2 className="mb-3 font-semibold text-white font-bangla">পেমেন্ট স্লিপ আপলোড করুন।</h2>
                <input
                  type="file"
                  id="ajaxUpload"
                  className="appearance-none w-full text-[15px] rounded-[10px] bg-[#fbfcff] shadow shadow-[#0057d0]/5 px-5 py-3.5 focus:outline-none focus:ring-1 focus:ring-[#0057d0]"
              ...
              <div className="font-bangla">
                <ul className="mt-5 text-slate-200">
                  <li className="flex text-sm">
                    <div><span className="bank-info-span"></span></div>
                    <div className="w-full flex justify-between">
                      <p>ব্যাংকের নামঃ <span className="text-yellow-300 font-semibold ml-1">{bankName}</span></p>
                    </div>
                  </li>
                  ...
                  <li className="flex text-sm">
                    <div><span className="bank-info-span"></span></div>
                    <div className="w-full flex justify-between">
                      <p>একাউন্ট নামঃ <span className="text-yellow-300 font-semibold ml-1">{bankAccountName}</span></p>
                      <CopyButton textToCopy={bankAccountName} className="px-2 py-0.5 mx-2 rounded-md bg-[#00000040] hover:bg-[#00000060]" />
                    </div>
                  </li>
                  <hr className="border-white/20 my-3" />
                  <li className="flex text-sm">
                    <div><span className="bank-info-span"></span></div>
                    <div className="w-full flex justify-between">
                      <p>একাউন্ট নম্বরঃ <span className="text-yellow-300 font-semibold ml-1">{bankAccountNumber}</span></p>
                      <CopyButton textToCopy={bankAccountNumber} className="px-2 py-0.5 mx-2 rounded-md bg-[#00000040] hover:bg-[#00000060]" />
                    </div>
                  </li>
                  <hr className="border-white/20 my-3" />
                  <li className="flex text-sm">
                    <div><span className="bank-info-span"></span></div>
                    <div className="w-full flex justify-between">
                      <p>ব্রাঞ্চ নামঃ <span className="text-yellow-300 font-semibold ml-1">{bankBranch}</span></p>
                      <CopyButton textToCopy={bankBranch} className="px-2 py-0.5 mx-2 rounded-md bg-[#00000040] hover:bg-[#00000060]" />
                    </div>
                  </li>
                  <hr className="border-white/20 my-3" />
                  <li className="flex text-sm">
                    <div><span className="bank-info-span"></span></div>
                    <div className="w-full flex justify-between">
                      <p>রাউটিং নম্বরঃ <span className="text-yellow-300 font-semibold ml-1">{bankRouting}</span></p>
                      <CopyButton textToCopy={bankRouting} className="px-2 py-0.5 mx-2 rounded-md bg-[#00000040] hover:bg-[#00000060]" />
                    </div>
                  </li>
                  <hr className="border-white/20 my-3" />
                  <li className="flex text-sm">
                    <div><span className="bank-info-span"></span></div>
                    <div className="w-full flex justify-between">
                      <p>সুইফট কোডঃ <span className="text-yellow-300 font-semibold ml-1">{bankSwift}</span></p>
                      <CopyButton textToCopy={bankSwift} className="px-2 py-0.5 mx-2 rounded-md bg-[#00000040] hover:bg-[#00000060]" />
                    </div>
                  </li>
                  <hr className="border-white/20 my-3" />
                  <li className="flex text-sm">
                    <div className="mr-2">
                      <svg width="15" viewBox="0 0 13 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M6.5 3.64V6.72M4.08 1H8.92L12 4.08V8.92L8.92 12H4.08L1 8.92V4.08L4.08 1Z" stroke="#FFED47" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"></path>
                        <circle cx="6.5" cy="8.5625" r="0.6875" fill="#FFED47"></circle>
                      </svg>
                    </div>
                    <p>উপরের উল্লেখিত তথ্য অনুযায়ী আপনার ব্যাংক ট্রান্সফার সম্পন্ন করুন। তারপর আপলোড বক্সে ব্যাংক স্লিপ অথবা ট্রান্সফার স্ক্রিনশট আপলোড করুন এবং নিচের<span className="text-yellow-300 font-semibold ml-1">SUBMIT</span> বাটনে ক্লিক করুন।</p>
                  </li>
                </ul>
              </div>
            </div>
            ...
            <div className="mt-5">
              <button
                ...
                className="bg-[#388E3C] hover:bg-[#4CAF50] disabled:opacity-50 disabled:cursor-not-allowed block rounded-[10px] px-4 py-3.5 text-center font-semibold text-white transition-colors w-full shadow-lg shadow-[#388E3C]/20"
              >
