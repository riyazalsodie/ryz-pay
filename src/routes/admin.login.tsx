  ...
  beforeLoad: async () => {
    ...
    try {
      ...
      if (
        ...
        (typeof e === "object" && e !== null && "to" in e)
      )
        throw e;
    }
  },
      ...
      <div className="w-full max-w-md p-8 bg-white/5 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/10 animate-in fade-in zoom-in duration-500 relative z-10">
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          <div className="space-y-2">
            ...
            <div className="relative group">
              <FontAwesomeIcon
                icon={faEnvelope}
                className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-500 group-focus-within:text-white transition-colors"
              />
              <input
                type="email"
            ...
            {errors?.email && (
              <p className="text-red-400 text-xs pl-1">{errors.email[0]}</p>
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            {errors?.password && (
              <p className="text-red-400 text-xs pl-1">{errors.password[0]}</p>
            )}
          </div>
        ...
        <div className="mt-8 pt-6 border-t border-white/10 text-center">
          <p className="text-gray-500 text-sm">
            Protected by RyzPay Security •{" "}
            <span className="text-white/60">256-bit Encryption</span>
          </p>
        </div>
      </div>
    </div>
